...
def fit_llaRef(lla, xyz):
    ...
    @jax.jit
    def llaRef_residual(llaRef):
        return lla2xyz(llaRef, lla, jnp) - jnp.array(xyz)

    @jax.jit
    def newton_step(llaRef):
        return llaRef - jnp.linalg.solve(jax.jacobian(llaRef_residual)(llaRef), llaRef_residual(llaRef))

    lat, lon, _ = lla
    inc = np.arange(-3, 4) / 2
    lat_ref, lon_ref, _ = min([(lat_ref, lon_ref, np.linalg.norm(llaRef_residual((lat_ref, lon_ref, 0))))
    ...
    return llaRef, llaRef_residual(llaRef)
...
class CoordinateConverter:
    ...
    def from_lla_and_xyz(cls, lla, xyz, np=np):
        llaRef, llaRef_residual = fit_llaRef(lla, xyz)
        if np.max(np.abs(llaRef_residual)) > 10:
            raise ValueError(f"Couldn't find a good `llaRef` fit for `lla` {lla} and `xyz` {xyz}.")
        converter = cls(np.array(llaRef), np)
        converter.llaRef_residual = llaRef_residual
        return converter
