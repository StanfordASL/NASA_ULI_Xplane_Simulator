...
class XPlaneConnect(object):
    ...
    def get_local_x(self):
        ...
        """
        return self.getDREF("sim/flightmodel/position/local_x")[0]

    def get_local_y(self):
        """
        Get the value of the aircraft's y-position in local coordinates
        """
        return self.getDREF("sim/flightmodel/position/local_y")[0]

    def get_local_z(self):
        """
        Get the value of the aircraft's z-position in local coordinates
        """
        return self.getDREF("sim/flightmodel/position/local_z")[0]

    def get_local_vx(self):
        """
        Get the value of the aircraft's x-velocity in local coordinates
        """
        return self.getDREF("sim/flightmodel/position/local_vx")[0]

    def get_local_vy(self):
        """
        Get the value of the aircraft's y-velocity in local coordinates
        """
        return self.getDREF("sim/flightmodel/position/local_vy")[0]

    def get_local_vz(self):
        """
        Get the value of the aircraft's z-velocity in local coordinates
        """
        return self.getDREF("sim/flightmodel/position/local_vz")[0]
    ...
    def get_heading(self):
        ...
        """
        return self.getDREF("sim/flightmodel/position/psi")[0]

    def get_ground_velocity(self):
        return self.getDREF("sim/flightmodel/position/groundspeed")

    # Utilities for sending common DREFs
    def send_local_x(self, x):
        """
        Set the value of the aircraft's x-position in local coordinates
        """
        return self.sendDREF("sim/flightmodel/position/local_x", x)

    def send_local_y(self, y):
        """
        Set the value of the aircraft's y-position in local coordinates
        """
        return self.sendDREF("sim/flightmodel/position/local_y", y)

    def send_local_z(self, z):
        """
        Set the value of the aircraft's z-position in local coordinates
        """
        return self.sendDREF("sim/flightmodel/position/local_z", z)

    def send_local_vx(self, vx):
        """
        Set the value of the aircraft's x-velocity in local coordinates
        """
        return self.sendDREF("sim/flightmodel/position/local_vx", vx)

    def send_local_vy(self, vy):
        """
        Set the value of the aircraft's y-velocity in local coordinates
        """
        return self.sendDREF("sim/flightmodel/position/local_vy", vy)

    def send_local_vz(self, vz):
        """
        Set the value of the aircraft's z-velocity in local coordinates
        """
        return self.sendDREF("sim/flightmodel/position/local_vz", vz)

    def send_heading(self, psi):
        """
        Set the value of the aircraft's heading in degrees from the Z axis
        """
        return self.sendDREF("sim/flightmodel/position/psi", psi)

    def send_true_heading(self, psi):
        p = [-998, -998, -998, -998, -998, psi, -998]
        return self.sendPOSI(p)

    def send_ground_velocity(self, v):
        """
        """
        psi = self.get_heading() * (math.pi/180)
        vx = v*math.sin(psi)
        vz = -v*math.cos(psi)

        return self.sendDREFs(["sim/flightmodel/position/local_vx", "sim/flightmodel/position/local_vz"], [vx, vz])

    def send_throttle(self, a):
        return self.sendCTRL([0, 0, 0, a])


    # Utilities for shifting current values to new ones
    def shift_local_x(self, dx):
        """
        Shift the value of the aircraft's x-position in local coordinates
        """
        x = self.get_local_x()
        return self.sendDREF("sim/flightmodel/position/local_x", x + dx)

    def shift_local_y(self, dy):
        """
        Shift the value of the aircraft's y-position in local coordinates
        """
        y = self.get_local_y()
        return self.sendDREF("sim/flightmodel/position/local_y", y + dy)

    def shift_local_z(self, dz):
        """
        Shift the value of the aircraft's z-position in local coordinates
        """
        z = self.get_local_z()
        return self.sendDREF("sim/flightmodel/position/local_z", z + dz)

    def shift_heading(self, dpsi):
        """
        Shift the value of the aircraft's heading in local coordinates
        """
        psi = self.get_heading()
        return self.send_heading(psi + dpsi)

    def shift_true_heading(self, dpsi):
        psi = self.getPOSI()[5]
        return self.send_true_heading(psi + dpsi)

    def shift_forward(self, d):
        x = self.get_local_x()
        z = self.get_local_z()
        psi = self.get_heading() * (math.pi/180)
        dx = d*math.sin(psi)
        ...
        return self.sendDREFs(["sim/flightmodel/position/local_x", "sim/flightmodel/position/local_z"], [x + dx, z + dz])
